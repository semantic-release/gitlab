...
module.exports = {
  EINVALIDGITLABURL: () => ({
    ...
    details: `The **semantic-release** \`repositoryUrl\` option must a valid GitLab URL with the format \`<GitHub_URL>/<repoId>.git\`.
...
By default the \`repositoryUrl\` option is retrieved from the \`repository\` property of your \`package.json\` or the [git origin url](https://git-scm.com/book/en/v2/Git-Basics-Working-with-Remotes) of the repository cloned by your CI environment.`,
  ...
  EINVALIDGLTOKEN: ({repoId}) => ({
    ...
    )}) configured in the \`GL_TOKEN\` or \`GITLAB_TOKEN\` environment variable must be a valid [personnal access token](https://docs.gitlab.com/ce/user/profile/personal_access_tokens.html) allowing to push to the repository ${repoId}.
...
Please make sure to create a [GitLab personnal access token](https://docs.gitlab.com/ce/user/profile/personal_access_tokens.html) and to set it in the \`GL_TOKEN\` or \`GITLAB_TOKEN\` environment variable on your CI environment. The token must allow to push to the repository ${repoId}.`,
  }),
};
