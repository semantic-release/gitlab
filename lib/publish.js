...
export default async (pluginConfig, context) => {
  ...
  const { gitlabToken, gitlabUrl, gitlabApiUrl, assets, milestones, proxy, retryLimit, retryStatusCodes, packageName } =
      ...
      allAssets.map(async (asset) => {
        ...
        if (_url) {
          ...
          if (target === "generic_package") {
            const pkgName = packageName ? template(packageName)(context) : "release";
            // Upload generic packages
            const encodedVersion = encodeURIComponent(version);
            const encodedPackageName = encodeURIComponent(pkgName);
            ...
            assetsList.push({ label, alt: pkgName, url, type: "package", filepath });
