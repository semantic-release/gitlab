...
module.exports = async (pluginConfig, context) => {
  ...
  if (assets && assets.length > 0) {
    ...
    await Promise.all(
      globbedAssets.map(async assetData => {
        const {path, label} = isPlainObject(assetData) ? assetData : {path: assetData};
        ...
        let {body} = await got.post(urlJoin(apiUrl, '/uploads'), {
          ...apiOptions,
          json: false,
          body: form,
        });
        ...
        assetsMd.push(`* [${label || body.alt}](${body.url})`);
